...
class FaceRecognitionService:
    ...
    def train_model(self, db):
        """
        Train model từ ảnh trong database (ưu tiên) hoặc từ file
        ...
        trained_count = 0
        face_db = FaceImageDB(db)
        
        for student in students:
            student_code = student['student_code']
            student_id = student['student_id']
            
            # Ưu tiên lấy ảnh từ database (face_image)
            image_from_db = face_db.get_face_image(student_id, as_array=True)
            
            if image_from_db is not None:
                # Có ảnh trong database
                ...
                try:
                    ...
                    gray = cv2.cvtColor(image_from_db, cv2.COLOR_BGR2GRAY)
                    ...
                    print(f"Lỗi xử lý ảnh từ DB cho {student_code}: {e}")
                    continue
            
            else:
                # Fallback: Đọc từ folder (legacy)
                face_folder = student.get('face_encoding_path')
